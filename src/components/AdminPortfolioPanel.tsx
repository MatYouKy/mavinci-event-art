    ...
    try {
      ...
      if (isNew) {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/portfolio-projects/${editingId}`,
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
      ...
      {isAdding && (
        ...
        >
          {({ submitForm, values, setFieldValue }) => (
            <Form className="rounded-xl border border-[#d3bb73]/20 bg-[#1c1f33] p-6">
              <h3 className="mb-4 text-xl font-light text-[#e5e4e2]">Nowy Projekt</h3>

              <div className="mb-6">
                <label className="mb-2 block text-sm font-medium text-[#e5e4e2]">Hero Image</label>
                <ImageEditorField
                  fieldName="imageData"
                  isAdmin={true}
                  mode="vertical"
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {projects.map((project) => {
          ...
          return (
            <div
              key={projectId}
              className="overflow-hidden rounded-xl border border-[#d3bb73]/20 bg-[#1c1f33]"
            >
              {editingId === projectId ? (
                <Formik
                  ...
                  <div className="p-4">
                    <div className="mb-2 flex items-start justify-between">
                      <div className="flex-1">
                        <h3 className="mb-1 text-lg font-medium text-[#e5e4e2]">{project.title}</h3>
                        <p className="text-sm text-[#d3bb73]">{project.category}</p>
                        <p className="mt-1 text-sm text-[#e5e4e2]/60">
                          Kolejność: {project.order_index}
                        </p>
                      </div>
                    ...
                    <p className="line-clamp-2 text-sm text-[#e5e4e2]/70">{project.description}</p>
                  </div>
                </>
              )}
            </div>
