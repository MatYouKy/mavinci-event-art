...
export default function EquipmentGallery({ equipmentId, canManage }: EquipmentGalleryProps) {
  ...
  const uploadFiles = async (files: FileList | File[]) => {
    ...
    for (let i = 0; i < filesArray.length; i++) {
      ...
      try {
        ...
        const { error: dbError } = await supabase.from('equipment_images').insert({
  ...
  const handleDelete = async (image: EquipmentImage) => {
    ...
    try {
      const urlParts = image.image_url.split('/equipment-images/');
      if (urlParts.length > 1) {
        const filePath = urlParts[1];
        await supabase.storage.from('equipment-images').remove([filePath]);
      }

      const { error } = await supabase.from('equipment_images').delete().eq('id', image.id);
            ...
            {images.map((image) => (
              ...
              >
                <div className="relative aspect-square">
                  <img
                    src={image.image_url}
                    alt={image.title || 'Equipment image'}
          ...
          >
            ...
            <div className="relative max-h-[80vh] max-w-full">
              <img
                ...
                alt={selectedImage.title || 'Equipment image'}
