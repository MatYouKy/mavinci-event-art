...
const toDatetimeLocalValue = (value: string | null): string => {
  ...
  // gdy wartość już pochodzi z inputa datetime-local
...
export default function EditEventModalNew({
  ...
  useEffect(() => {
    ...
    setClientData({
      client_type:
        event?.client_type || (contact?.contact_type === 'individual' ? 'individual' : 'business'),
      organization_id: event?.organization_id || null,
      contact_person_id: event?.contact_person_id || contact?.id || null,
    });
  ...
  const handleSubmit = () => {
    ...
    if (!formData.event_date) {
      return showAlert('Data rozpoczęcia jest wymagana');
    }

    if (!formData.location.trim()) {
      return showAlert('Lokalizacja jest wymagana');
    }
        ...
        <div className="flex min-h-full w-full items-start justify-center p-4 sm:p-6">
          <div className="max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto rounded-xl border border-[#d3bb73]/20 bg-[#0f1119] shadow-2xl">
            ...
            <div className="space-y-6 p-6">
              ...
              <ClientSelectorTabs
                key={`${clientData.client_type}-${clientData.organization_id}-${clientData.contact_person_id}`}
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={toDatetimeLocalValue(formData.event_date)}
                    onChange={(e) =>
                      setFormData((prev) => ({
                        ...prev,
                ...
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={toDatetimeLocalValue(formData.event_end_date)}
                    onChange={(e) =>
                      setFormData((prev) => ({
                        ...prev,
                        event_end_date: e.target.value || null,
                      }))
                    }
                    className="w-full rounded-lg border border-[#d3bb73]/20 bg-[#1c1f33] px-4 py-2 text-[#e5e4e2] focus:border-[#d3bb73] focus:outline-none"
                  />
