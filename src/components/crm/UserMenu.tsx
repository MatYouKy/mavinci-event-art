...
export default function UserMenu({
  ...
  // ✅ opcjonalnie: cichy refresh (nie pokazuj loadera, tylko podmień dane jeśli przyszły inne)
  useEffect(() => {
    refreshUserDataSilently();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const refreshUserDataSilently = async () => {
    try {
      const {
        data: { session },
      ...
      // ignorujemy – SSR już pokazał poprawne UI
  ...
  const getInitials = () => {
    if (employee) return `${employee.name.charAt(0)}${employee.surname.charAt(0)}`.toUpperCase();
