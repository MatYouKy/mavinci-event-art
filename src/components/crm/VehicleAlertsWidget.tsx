  ...
  return (
    <div className="space-y-2">
      ...
      <button
        ...
        className="flex w-full items-center justify-between gap-4 rounded-lg border border-[#e5e4e2]/20 bg-[#e5e4e2]/5 px-4 py-3 transition-colors hover:bg-[#e5e4e2]/10"
      >
        <div className="flex items-center gap-3">
          <AlertTriangle className="h-5 w-5 text-orange-400" />
          <div className="text-left">
            <div className="font-medium text-[#e5e4e2]">Aktywne alerty: {alerts.length}</div>
            {blockingAlertsCount > 0 && (
              <div className="text-sm text-red-400">{blockingAlertsCount} blokujących</div>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          {alerts.slice(0, 3).map((alert) => (
            <span
              key={alert.id}
              className={`rounded px-2 py-1 text-xs ${getPriorityStyles(alert.priority)}`}
            >
        ...
        <div className="space-y-2">
          {alerts.map((alert) => (
            <div
              ...
              className={`cursor-pointer rounded-lg border p-4 transition-colors hover:bg-[#e5e4e2]/5 ${getPriorityStyles(
                alert.priority,
              )}`}
            >
              <div className="flex items-start gap-3">
                <div className="mt-0.5 flex-shrink-0">{getIcon(alert.icon)}</div>
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex items-center gap-2">
                    <h4 className="font-medium">{alert.title}</h4>
                    {alert.is_blocking && (
                      <span className="rounded border border-red-500/50 bg-red-500/30 px-2 py-0.5 text-xs text-red-300">
                        🔒 Blokuje
                      </span>
                    )}
                  </div>
                ...
                <div>
                  <span
                    className={`rounded border px-2 py-1 text-xs ${getPriorityStyles(alert.priority)}`}
                  >
                    {getPriorityLabel(alert.priority)}
                  </span>
                </div>
