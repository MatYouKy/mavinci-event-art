...
export const fetchConnectorTypes = createAsyncThunk('equipment/fetchConnectorTypes', async () => {
  const { data, error } = await supabase
...
export const fetchEquipmentDetails = createAsyncThunk(
  ...
  async (
    { equipmentId, includeUnits }: { equipmentId: string; includeUnits: boolean },
    { rejectWithValue },
  ) => {
    ...
    const { data: categories } = await supabase.from('warehouse_categories').select('*');
...
export const logEquipmentEdit = createAsyncThunk(
  ...
  async (p: {
    ...
    const { error } = await supabase.from('equipment_edit_history').insert({
...
export const addEquipmentComponent = createAsyncThunk(
  ...
  async ({
    ...
    equipment_id: string;
    component: Omit<EquipmentComponent, 'id' | 'created_at' | 'equipment_id'> & {
...
export const updateEquipmentStockSettings = createAsyncThunk(
  ...
  async ({
    ...
    stock_id: string;
    payload: Pick<EquipmentStock, 'storage_location' | 'min_stock_level'>;
  }) => {
    const { error } = await supabase
...
export const addEquipmentUnitEvent = createAsyncThunk(
  ...
  async (p: { unit: EquipmentUnit; event: Omit<EquipmentUnitEvent, 'id' | 'created_at'> }) => {
    // 1) Insert event
    const { error } = await supabase.from('equipment_unit_events').insert({
      unit_id: p.unit.id,
...
export const upsertEquipmentUnit = createAsyncThunk(
  ...
  async (payload: { id?: string; equipment_id: string; [key: string]: any }) => {
    if (payload.id) {
      ...
      const { error } = await supabase.from('equipment_units').update(updateData).eq('id', id);
      if (error) throw error;
    } else {
      const { error } = await supabase.from('equipment_units').insert(payload);
      if (error) throw error;
    }
    return true;
  },
...
export const addUnitEvent = createAsyncThunk(
  ...
  async (payload: {
    ...
    const { error } = await supabase.from('equipment_unit_events').insert(payload);
    if (error) throw error;
    return true;
  },
);
