...
export default function SymulatoryVRPage() {
  ...
  const fetchData = async () => {
    try {
      const [introRes, equipmentRes, featuresRes, galleryRes, experiencesRes, benefitsRes] = await Promise.all([
        supabase.from('vr_simulators_intro').select('*').single(),
        supabase.from('vr_equipment_types').select('*').eq('is_visible', true).order('order_index'),
                        ...
                        {Object.entries(item.specs).map(([key, value]: [string, any]) => (
                          <div key={key} className="flex justify-between">
                            ...
                            <span className="font-medium text-[#e5e4e2]/60">
                              {Array.isArray(value) ? value.join(', ') : typeof value === 'boolean' ? (value ? 'Tak' : 'Nie') : value}
                            </span>
                          </div>
                        ))}
                      </div>
