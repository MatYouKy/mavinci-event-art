...
export default function TechnicalStageFeatures() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'technical_stage_features' }, (payload) => {
        if (payload.eventType === 'INSERT') {
          setFeatures(prev => [...prev, payload.new as Feature].sort((a, b) => a.order_index - b.order_index));
        } else if (payload.eventType === 'UPDATE') {
          setFeatures(prev => prev.map(f => f.id === payload.new.id ? payload.new as Feature : f));
