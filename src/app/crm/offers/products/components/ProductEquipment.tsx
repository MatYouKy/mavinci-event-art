...
export const ProductEquipment: FC<IProductEquipment> = ({
  ...
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set(['required', 'optional']));
  ...
  const groupedByOptional = useMemo(() => {
    ...
    const result: Array<{ key: string; title: string; items: ProductEquipmentViewRow[]; color: string }> = [];
