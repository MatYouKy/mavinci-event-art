...
export const clientsApi = createApi({
  ...
  endpoints: (builder) => ({
        ...
        try {
          ...
          for (const r of rels || []) {
            if (r.organization_id)
              orgContactCounts.set(r.organization_id, (orgContactCounts.get(r.organization_id) || 0) + 1);
            if (r.contact_id)
          ...
          if (tab === 'all' || tab === 'clients' || tab === 'subcontractors') {
            ...
            let orgQuery = supabase.from('organizations').select('*').order('created_at', { ascending: false });
    ...
    getOrganizationById: builder.query<OrganizationRow, UUID>({
      ...
      providesTags: (_res, _err, id) => [{ type: 'Org', id }, { type: 'ClientById', id }],
    }),
    ...
    getContactById: builder.query<ContactRow, UUID>({
      ...
      providesTags: (_res, _err, id) => [{ type: 'Contact', id }, { type: 'ClientById', id }],
    }),
    ...
    // ---------- UPDATE ----------
    updateOrganizationById: builder.mutation<OrganizationRow, { id: UUID; data: UpdateOrganizationPayload }>({
