...
export default function ConferencesGalleryEditor() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'conferences_gallery' },
        () => {
          fetchImages();
        },
      )
  ...
  const handleSetPrimary = async (imageId: string) => {
    try {
      await supabase.from('conferences_gallery').update({ is_primary: false });
        ...
        >
          <div className="mb-8 flex items-center justify-between">
            <div className="flex-1 text-center">
              ...
              <p className="mb-8 text-[#e5e4e2]/60">Galeria zdjęć z obsłużonych konferencji</p>
            </div>
            {canEdit && (
                  ...
                  {(image.title || image.caption) && (
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                      <div className="absolute bottom-0 left-0 right-0 p-3">
                        {image.title && (
                          <h3 className="mb-1 text-sm font-medium text-white">{image.title}</h3>
                        )}
                        {image.caption && <p className="text-xs text-white/80">{image.caption}</p>}
                      </div>
                    </div>
                  )}
                </motion.div>
