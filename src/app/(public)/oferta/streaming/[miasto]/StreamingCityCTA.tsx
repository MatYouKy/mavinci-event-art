...
export default function StreamingCityCTA({ cityCases, content }: Props) {
  const prep = cityCases.locative_preposition || 'w';

  const heading =
    content?.cta_heading ||
    `Potrzebujesz streamingu ${prep} ${capitalize(cityCases.locative)}?`;

  const description =
    content?.cta_description ||
    `Skontaktuj się z nami - przygotujemy indywidualną wycenę transmisji dopasowaną do Twojego wydarzenia ${prep} ${capitalize(cityCases.locative)}. Bezpłatna konsultacja i oferta w 24h.`;
