...
export default function QuizFormatEditModal({ formatId, onClose }: QuizFormatEditModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formatId) {
        ...
        // Create new
        const { error } = await supabase.from('quiz_show_formats').insert({
          title,
          level,
          description,
          features: filteredFeatures,
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      const { error } = await supabase.from('quiz_show_formats').delete().eq('id', formatId);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <div className="relative max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-[#d3bb73]/20 bg-[#1c1f33] p-8">
        {/* Header */}
        ...
        <div className="space-y-6">
          {/* Title */}
          <div>
            <label className="mb-2 block text-sm font-light text-[#e5e4e2]/80">Tytuł *</label>
            <input
              type="text"
              value={title}
          ...
          {/* Level */}
          <div>
            <label className="mb-2 block text-sm font-light text-[#e5e4e2]/80">Poziom *</label>
            <input
              type="text"
              value={level}
          ...
          {/* Description */}
          <div>
            <label className="mb-2 block text-sm font-light text-[#e5e4e2]/80">Opis *</label>
            <textarea
              value={description}
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              <label className="text-sm font-light text-[#e5e4e2]/80">Cechy *</label>
              <button
                onClick={addFeature}
          ...
          {/* Image */}
          <div>
            <label className="mb-2 block text-sm font-light text-[#e5e4e2]/80">Zdjęcie</label>
            <SimpleImageUploader
          ...
          {/* Icon */}
          <div>
            <label className="mb-2 block text-sm font-light text-[#e5e4e2]/80">Ikona</label>
          ...
          <div>
            <label className="flex cursor-pointer items-center gap-3">
              ...
              />
              <span className="text-sm font-light text-[#e5e4e2]/80">Widoczny na stronie</span>
            </label>
          </div>
        </div>
