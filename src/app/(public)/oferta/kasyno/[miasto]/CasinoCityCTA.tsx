...
export default function CasinoCityCTA({ cityCases, content }: Props) {
  const prep = cityCases.locative_preposition || 'w';

  const heading =
    content?.cta_heading ||
    `Potrzebujesz kasyna rozrywkowego ${prep} ${capitalize(cityCases.locative)}?`;

  const description =
    content?.cta_description ||
    `Skontaktuj się z nami - przygotujemy indywidualną wycenę kasyna rozrywkowego dopasowaną do Twojego wydarzenia ${prep} ${capitalize(cityCases.locative)}. Bezpłatna konsultacja i oferta w 48h.`;
    ...
    <section className="px-6 py-24">
      <div className="mx-auto max-w-4xl rounded-2xl border border-[#d3bb73]/30 bg-gradient-to-br from-[#1c1f33] to-[#0f1119] p-12 text-center">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-6 text-sm text-[#e5e4e2]/50">
          ...
          <span>Wycena w 48h</span>
          <span className="text-[#d3bb73]/30">|</span>
          <span>Profesjonalni krupierzy</span>
          <span className="text-[#d3bb73]/30">|</span>
          <span>Wlasny sprzet</span>
        </div>
      </div>
    </section>
  );
