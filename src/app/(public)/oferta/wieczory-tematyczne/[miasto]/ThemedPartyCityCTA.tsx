...
export default function ThemedPartyCityCTA({ cityCases, content }: Props) {
  const prep = cityCases.locative_preposition || 'w';

  const heading =
    content?.cta_heading ||
    `Planujesz wieczór tematyczny ${prep} ${capitalize(cityCases.locative)}?`;

  const description =
    content?.cta_description ||
    `Skontaktuj się z nami - przygotujemy indywidualną koncepcję i wycenę wieczoru tematycznego dopasowaną do Twojego wydarzenia ${prep} ${capitalize(cityCases.locative)}. Bezpłatna konsultacja i oferta w 24h.`;
    ...
    <section className="px-6 py-24">
      <div className="mx-auto max-w-4xl rounded-2xl border border-[#d3bb73]/30 bg-gradient-to-br from-[#1c1f33] to-[#0f1119] p-12 text-center">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-6 text-sm text-[#e5e4e2]/50">
          ...
          <span className="text-[#d3bb73]/30">|</span>
          <span>Wlasny sprzet i dekoracje</span>
          <span className="text-[#d3bb73]/30">|</span>
          <span>Realizacja w calej Polsce</span>
        </div>
      </div>
    </section>
  );
