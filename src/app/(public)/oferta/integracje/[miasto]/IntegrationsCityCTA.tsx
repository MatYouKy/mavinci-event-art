...
export default function IntegrationsCityCTA({ cityCases, content }: Props) {
  const prep = cityCases.locative_preposition || 'w';

  const heading =
    content?.cta_heading ||
    `Planujesz integracje firmowa ${prep} ${capitalize(cityCases.locative)}?`;

  const description =
    content?.cta_description ||
    `Skontaktuj sie z nami - przygotujemy indywidualny scenariusz integracji dopasowany do Twojego zespolu ${prep} ${capitalize(cityCases.locative)}. Bezplatna konsultacja i wycena w 24h.`;
    ...
    <section className="px-6 py-24">
      <div className="mx-auto max-w-4xl rounded-2xl border border-[#d3bb73]/30 bg-gradient-to-br from-[#1c1f33] to-[#0f1119] p-12 text-center">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-6 text-sm text-[#e5e4e2]/50">
          ...
          <span className="text-[#d3bb73]/30">|</span>
          <span>Doswiadczeni animatorzy</span>
          <span className="text-[#d3bb73]/30">|</span>
