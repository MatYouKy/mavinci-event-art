...
export async function generateMetadata({
  ...
  const ogImageUrl =
    heroImage?.image_url || service.thumbnail_url || 'https://mavinci.pl/logo-mavinci-crm.png';
...
export default async function ServiceDetailPage({ params }: { params: { slug: string } }) {
  ...
  const seo = await getSeoForPage(`uslugi/${params.slug}`, cookies());
  if (!seo) {
    notFound();
  }

  const { service, category, relatedServices, heroImage, globalConfig, gallery, hasWebsiteEdit } =
    data;
