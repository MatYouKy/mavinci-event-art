...
export async function GET(req: Request) {
  ...
  if (assignment.status !== "pending") {
    return NextResponse.redirect(new URL("/invitation/error?reason=already-responded", origin), 302);
  }
  ...
  return NextResponse.redirect(new URL("/invitation/rejected", origin), 302);
}