        ...
        {filteredContracts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredContracts.map((contract) => (
              <div
                key={contract.id}
                className="flex cursor-pointer flex-col rounded-xl border border-[#d3bb73]/10 bg-[#1c1f33] p-6 transition-colors hover:border-[#d3bb73]/30"
                onClick={() => router.push(`/crm/contracts/${contract.id}`)}
              >
                <div className="mb-3 flex items-center justify-between">
                  <FileText className="h-8 w-8 text-[#d3bb73]" />
                  <span className={`rounded px-2 py-1 text-xs ${statusColors[contract.status]}`}>
                    {statusLabels[contract.status]}
                  </span>
                </div>

                <h3 className="mb-1 text-base font-medium text-[#e5e4e2]">
                  {contract.contract_number}
                </h3>
                <p className="mb-3 line-clamp-2 flex-1 text-sm text-[#e5e4e2]/80">
                  {contract.title}
                </p>

                <div className="mb-4 space-y-2 text-xs text-[#e5e4e2]/60">
                ...
                <div className="flex items-center gap-2 border-t border-[#d3bb73]/10 pt-3">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      router.push(`/crm/contracts/${contract.id}`);
                    }}
                    className="flex-1 rounded-lg p-2 text-center text-[#d3bb73] transition-colors hover:bg-[#d3bb73]/10"
                    title="Szczegóły"
                  >
                  ...
                  {contract.pdf_url && (
                    <button
                      ...
                      className="flex-1 rounded-lg p-2 text-center text-blue-400 transition-colors hover:bg-blue-400/10"
                      title="Pobierz PDF"
                    >
                  ...
                  <button
                    ...
                    className="flex-1 rounded-lg p-2 text-center text-red-400 transition-colors hover:bg-red-400/10"
                    title="Usuń"
                  >
