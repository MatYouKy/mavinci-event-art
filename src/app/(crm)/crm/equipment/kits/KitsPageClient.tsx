...
export function KitsPageClient({ viewMode }: { viewMode: ViewMode }) {
  ...
  const handleViewModeChange = async (mode: 'grid' | 'list' | 'table') => {
    setLocalViewMode(mode);
    await setViewMode('kits', mode as ViewMode);
  };
            ...
            <div className="space-y-3">
              {viewingKit.equipment_kit_items.map((item, index) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="flex items-center gap-4 rounded-lg bg-[#1c1f33] p-2"
                  >
                        ...
                        <Popover
                          trigger={
                            <Image
                              ...
                              onClick={() => {
                                if (displayItem.id) {
                                  router.push(`/crm/equipment/${displayItem.id}`);
                                }
                              }}
                              width={20}
                              height={20}
                              className="mb-4 h-10 w-10 cursor-pointer rounded-lg object-cover"
                            />
                          }
                          content={
                            <Image
                    ...
                    <div className="flex-1">
                      <div
                        className="cursor-pointer font-medium text-[#e5e4e2] hover:text-[#d3bb73]"
                        onClick={() => {
                          if (displayItem.id) {
                            router.push(`/crm/equipment/${displayItem.id}`);
                          }
    ...
    <div className="space-y-3">
      {filtered.map((kit) => {
        ...
        return (
          <div
            key={kit.id}
            className="flex flex-col gap-4 rounded-lg border border-[#d3bb73]/10 bg-[#1c1f33] p-4 transition-colors hover:border-[#d3bb73]/30 md:flex-row md:items-center"
          >
            <div
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <h3 className="truncate text-lg font-medium text-[#e5e4e2]">{kit.name}</h3>

                <div className="mt-1 flex flex-wrap items-center gap-2 text-sm">
                  {category && <span className="text-[#d3bb73]">{category.name}</span>}
                ...
                {kit.description && (
                  <p className="mt-2 line-clamp-2 text-sm text-[#e5e4e2]/60">{kit.description}</p>
                )}
              </div>
            </div>
            ...
            {canManage && (
              <div className="flex gap-2 md:justify-end">
                <button
                  ...
                  className="flex flex-1 items-center justify-center gap-2 rounded border border-[#d3bb73]/10 bg-[#0f1119] px-3 py-2 text-[#e5e4e2] hover:border-[#d3bb73]/30 md:flex-none"
                >
                  <Edit className="h-4 w-4" />
                  Edytuj
                </button>
        ...
        <table className="w-full min-w-[760px]">
          <thead className="bg-[#0f1119]">
            <tr className="border-b border-[#d3bb73]/10">
              ...
              {canManage && (
                <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-[#e5e4e2]/50">
                  Akcje
                </th>
              )}
            </tr>
          ...
          <tbody>
            {filtered.map((kit) => {
              ...
              return (
                <tr
                  key={kit.id}
                  className="border-b border-[#d3bb73]/5 transition-colors hover:bg-[#0f1119]"
                >
                  <td className="px-4 py-3">
                    ...
                    >
                      {kit.thumbnail_url ? (
                        ...
                        <div className="flex h-12 w-12 items-center justify-center rounded bg-[#0f1119]">
                          <Package className="h-5 w-5 text-[#e5e4e2]/20" />
                        </div>
                      )}

                      <div className="min-w-0">
                        <div className="truncate font-medium text-[#e5e4e2]">{kit.name}</div>
                      </div>
                    </div>
                  </td>

                  <td className="px-4 py-3 text-sm text-[#d3bb73]">{category?.name || '—'}</td>

                  <td className="max-w-[280px] px-4 py-3 text-sm text-[#e5e4e2]/60">
                    <div className="truncate">{kit.description || '—'}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-sm text-[#e5e4e2]/50">
                    {new Date(kit.created_at).toLocaleDateString('pl-PL')}
                  </td>
                  ...
                  {canManage && (
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => handleOpenForm(kit)}
                          className="flex items-center gap-2 rounded border border-[#d3bb73]/10 bg-[#0f1119] px-3 py-2 text-[#e5e4e2] hover:border-[#d3bb73]/30"
                        >
                          <Edit className="h-4 w-4" />
                          Edytuj
                        </button>
            ...
            <div className="p-4">
              <div className="mb-2 flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {category && <div className="mt-1 text-xs text-[#d3bb73]">{category.name}</div>}
                </div>
              </div>
              {kit.description && (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-[#d3bb73]/10 bg-[#1c1f33] p-1">
          <button
            onClick={() => handleViewModeChange('grid')}
            className={`flex items-center gap-2 rounded-md px-3 py-2 text-sm transition-colors ${
              localViewMode === 'grid'
                ? 'bg-[#d3bb73] text-[#1c1f33]'
                : 'text-[#e5e4e2]/70 hover:bg-[#0f1119]'
            }`}
          >
