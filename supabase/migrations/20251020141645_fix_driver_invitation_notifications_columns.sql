/*
  # Napraw kolumny w triggerach zaproszeń kierowców
  
  Problem:
  - Triggery używają entity_type i entity_id
  - Tabela notifications ma related_entity_type i related_entity_id
  
  Rozwiązanie:
  - Dostosuj wszystkie INSERT do notifications używając prawidłowych nazw kolumn
*/
...
BEGIN
  ...
  IF NEW.driver_id IS NOT NULL AND (TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND NEW.driver_id IS DISTINCT FROM OLD.driver_id)) THEN
    -- Wstaw notyfikację z poprawnymi nazwami kolumn
    ...
    -- Wstaw odbiorcę (employee_id zamiast user_id)
