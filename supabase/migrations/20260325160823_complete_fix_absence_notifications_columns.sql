/*
  # Complete Fix for Absence Notification Functions

  1. Changes
    - Fix approve_absence to use correct column names (title, related_entity_type, related_entity_id, user_id)
    - Fix reject_absence to use correct column names
    - Ensure all notification functions use the correct schema

  2. Notification table structure:
    - title (NOT subject)
    - related_entity_type (NOT entity_type)
    - related_entity_id (NOT entity_id)
    
  3. Notification_recipients table structure:
    - user_id (NOT employee_id)
*/

-- Funkcja do zatwierdzania nieobecności (COMPLETE FIX)
...
BEGIN
  ...
  -- Wyślij notyfikację do pracownika o zatwierdzeniu (FIXED)
  ...
  -- Dodaj odbiorcę - pracownika (FIXED: użyj user_id)
...
-- Funkcja do odrzucania nieobecności (COMPLETE FIX)
...
BEGIN
  ...
  -- Wyślij notyfikację do pracownika o odrzuceniu (FIXED)
  INSERT INTO notifications (
    category,
    title,
    message,
