/*
  # Fix conflict checker - use correct equipment_units column name

  1. Changes
    - Use `equipment_id` instead of `equipment_item_id` in equipment_units queries

  2. Notes
    - equipment_units table has `equipment_id` not `equipment_item_id`
*/
...
BEGIN
  ...
  LOOP
    ...
    -- Get item availability (FIXED: use equipment_id not equipment_item_id)
    SELECT
      ei.name,
