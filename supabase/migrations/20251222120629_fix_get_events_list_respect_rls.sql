/*
  ...
  1. Changes
    - Change get_events_list from SECURITY DEFINER to SECURITY INVOKER
    - This will make the function respect the RLS policies we set up
    - Users with 'calendar_manage' will see all events
    - Users with 'calendar_view' will only see events they are assigned to
    - Users without calendar permissions will see no events

  2. Security
    - Function now runs with caller's permissions
    - Respects events table RLS policies
    - Ensures users only see events they have access to
*/
