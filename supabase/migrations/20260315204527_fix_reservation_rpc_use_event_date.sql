/*
  # Naprawa Funkcji RPC - Użycie Prawidłowych Kolumn

  1. Problem
    - Błąd: column evt.start_date does not exist
    - Tabela events ma event_date i event_end_date, nie start_date/end_date

  2. Rozwiązanie
    - Zmiana evt.start_date → evt.event_date
    - Zmiana evt.end_date → evt.event_end_date
    - Zmiana conflict_evt.start_date → conflict_evt.event_date
    - Zmiana conflict_evt.end_date → conflict_evt.event_end_date
*/
...
DECLARE
  ...
  v_item_name text;
BEGIN
  -- Pobierz dane eventu - UŻYJ event_date i event_end_date
  SELECT evt.id, evt.event_date, evt.event_end_date
      ...
      SELECT
        ...
        COALESCE(SUM(
          CASE WHEN ee.event_id != v_event_id
               AND conflict_evt.event_date < v_end_date
               AND conflict_evt.event_end_date > v_start_date
...
'Zwraca listę sprzętu z oferty wraz z aktualną dostępnością - używa event_date i event_end_date';
