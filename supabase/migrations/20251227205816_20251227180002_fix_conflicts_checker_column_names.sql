/*
  # Fix conflict checker - correct column names for events table

  1. Changes
    - Use `event_date` instead of `start_date`
    - Use `event_end_date` instead of `end_date`

  2. Notes
    - This matches the actual events table structure
*/
...
DECLARE
  ...
  i integer;
BEGIN
  -- Get event dates (correct column names)
  ...
  LOOP
    ...
    -- Get item availability (correct column names)
    SELECT
      ei.name,
