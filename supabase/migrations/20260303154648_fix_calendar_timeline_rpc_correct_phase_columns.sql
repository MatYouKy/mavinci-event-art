/*
  # Fix Calendar Timeline RPC - Use correct event_phases column names

  1. Changes
    - Replace work_start_datetime with start_time
    - Replace work_end_datetime with end_time
    - These are the actual column names in event_phases table
*/
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(ep.start_time, e.event_start_datetime, e.event_date) as event_start,
    COALESCE(ep.end_time, e.event_end_datetime, e.event_end_date) as event_end,
  ...
  WHERE v.status IN ('available', 'in_use', 'maintenance')
    AND (
      ev.id IS NULL
      OR (
        COALESCE(ep.end_time, e.event_end_datetime, e.event_end_date) >= p_start_date
        AND COALESCE(ep.start_time, e.event_start_datetime, e.event_date) <= p_end_date
