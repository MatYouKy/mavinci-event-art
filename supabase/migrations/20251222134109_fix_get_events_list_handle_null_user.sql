/*
  # Fix get_events_list - handle NULL user properly
  
  1. Problem
    - When auth.uid() is NULL (service role or no session), function returns 0 results
    - Need to handle NULL user case properly
  
  2. Solution
    - If user is NULL, return empty array (no access)
    - If user exists, check permissions and filter accordingly
*/
...
COMMENT ON FUNCTION get_events_list IS 'Returns filtered list of events and meetings based on user permissions';
