/*
  # Fix Employee Timeline - Use approval_status Column

  Naprawia funkcję get_employee_timeline_data aby używała:
  - employee_absences.approval_status (nie status)

  Problem: funkcja odwołuje się do kolumny "status" która może nie istnieć
  lub nazywa się "approval_status" w tabeli employee_absences.
*/
...
BEGIN
  RETURN QUERY
  -- Przypisania do FAZ (phase assignments) - z invitation_status
  SELECT
    e.id as employee_id,
  ...
  -- Przypisania do CAŁYCH WYDARZEŃ (employee_assignments) - z status
  SELECT
    e.id as employee_id,
    (e.name || ' ' || e.surname)::text as employee_name,
    'event'::text as item_type,
    ...
    ev.status as item_status,
    -- Kolor zależy od status z employee_assignments
    CASE
      WHEN ea.status = 'pending' THEN '#fbbf24'
      WHEN ea.status = 'accepted' THEN COALESCE(ec.color, '#10b981')
      WHEN ea.status = 'rejected' THEN '#ef4444'
      ELSE COALESCE(ec.color, '#3b82f6')
    END as item_color,
    jsonb_build_object(
      'category', ec.name,
      'location', l.name,
      'invitation_status', ea.status,
      'role', ea.role,
  ...
  -- Nieobecności - używamy approval_status zamiast status
  SELECT
    e.id as employee_id,
    (e.name || ' ' || e.surname)::text as employee_name,
