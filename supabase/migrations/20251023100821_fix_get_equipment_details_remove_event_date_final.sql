/*
  # Fix get_equipment_details - remove event_date references
  
  Changes:
  - DROP both versions of get_equipment_details
  - Recreate with correct signature (item_id, include_units)
  - Use created_at instead of event_date
  - Fix all column references for equipment_unit_events
*/

-- Drop all versions
DROP FUNCTION IF EXISTS get_equipment_details(uuid);
  ...
  SELECT jsonb_build_object(
    ...
    'equipment_stock', (
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
          'id', es.id,
          'equipment_item_id', es.equipment_item_id,
          'storage_location_id', es.storage_location_id,
      ...
      WHERE es.equipment_item_id = item_id
    ),
    'equipment_components', (
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
          'id', ec.id,
          'equipment_item_id', ec.equipment_item_id,
      ...
      WHERE ec.equipment_item_id = item_id
    ),
    'equipment_images', (
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
          ...
          'display_order', ei_img.display_order,
          'created_at', ei_img.created_at
        ) ORDER BY ei_img.display_order
      ), '[]'::jsonb)
      FROM equipment_images ei_img
      WHERE ei_img.equipment_item_id = item_id
    ),
    'units', CASE WHEN include_units THEN
      (
        ...
        WHERE eu.equipment_item_id = item_id
      )
    ELSE '[]'::jsonb
    END,
    'unit_events', (
