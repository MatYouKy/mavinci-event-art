...
BEGIN
  ...
  IF NEW.price_net IS NOT NULL AND (OLD.price_net IS NULL OR NEW.price_net != OLD.price_net OR NEW.vat_rate != OLD.vat_rate) THEN
    ...
    NEW.base_price := NEW.price_net; -- dla kompatybilności wstecznej
  END IF;
  ...
  IF NEW.cost_net IS NOT NULL AND (OLD.cost_net IS NULL OR NEW.cost_net != OLD.cost_net OR NEW.vat_rate != OLD.vat_rate) THEN
    ...
    NEW.cost_price := NEW.cost_net; -- dla kompatybilności wstecznej
  END IF;
  ...
  IF NEW.transport_cost_net IS NOT NULL AND (OLD.transport_cost_net IS NULL OR NEW.transport_cost_net != OLD.transport_cost_net OR NEW.vat_rate != OLD.vat_rate) THEN
    ...
    NEW.transport_cost := NEW.transport_cost_net; -- dla kompatybilności wstecznej
  END IF;
  ...
  IF NEW.logistics_cost_net IS NOT NULL AND (OLD.logistics_cost_net IS NULL OR NEW.logistics_cost_net != OLD.logistics_cost_net OR NEW.vat_rate != OLD.vat_rate) THEN
    ...
    NEW.logistics_cost := NEW.logistics_cost_net; -- dla kompatybilności wstecznej
  END IF;
...
-- Funkcja do przeliczania VAT dla produktu (z ceny brutto na netto)
CREATE OR REPLACE FUNCTION calculate_product_prices_from_gross()
RETURNS TRIGGER AS $$
BEGIN
  -- Jeśli zmieniono cenę brutto (i nie zmieniono netto), przelicz netto
  IF NEW.price_gross IS NOT NULL AND (OLD.price_net = NEW.price_net OR OLD.price_net IS NULL) AND NEW.price_gross != OLD.price_gross THEN
    NEW.price_net := NEW.price_gross / (1 + NEW.vat_rate / 100);
    NEW.base_price := NEW.price_net;
  END IF;

  IF NEW.cost_gross IS NOT NULL AND (OLD.cost_net = NEW.cost_net OR OLD.cost_net IS NULL) AND NEW.cost_gross != OLD.cost_gross THEN
    NEW.cost_net := NEW.cost_gross / (1 + NEW.vat_rate / 100);
    NEW.cost_price := NEW.cost_net;
  END IF;

  IF NEW.transport_cost_gross IS NOT NULL AND (OLD.transport_cost_net = NEW.transport_cost_net OR OLD.transport_cost_net IS NULL) AND NEW.transport_cost_gross != OLD.transport_cost_gross THEN
    NEW.transport_cost_net := NEW.transport_cost_gross / (1 + NEW.vat_rate / 100);
    NEW.transport_cost := NEW.transport_cost_net;
  END IF;

  IF NEW.logistics_cost_gross IS NOT NULL AND (OLD.logistics_cost_net = NEW.logistics_cost_net OR OLD.logistics_cost_net IS NULL) AND NEW.logistics_cost_gross != OLD.logistics_cost_gross THEN
    NEW.logistics_cost_net := NEW.logistics_cost_gross / (1 + NEW.vat_rate / 100);
