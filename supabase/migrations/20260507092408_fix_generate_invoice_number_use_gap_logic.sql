/*
  # Align generate_invoice_number with peek_next_invoice_number

  1. Problem
    - generate_invoice_number blindly incremented my_companies counters
    - Refresh button repeatedly called it, bumping counter each click (n+1 bug)
    - Counters drifted from actual invoice numbers in use
  2. Fix
    - Rewrite two-arg generate_invoice_number to compute first free number by scanning
      existing invoices + ksef_invoices for the given company/prefix/year, just like peek
    - Still updates my_companies counter to MAX(used, returned) for bookkeeping
    - Year reset still clears counters when year rolls over
*/
