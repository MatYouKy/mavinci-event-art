/*
  # Fix conflict checker - use correct event_equipment column name

  1. Changes
    - Use `equipment_id` instead of `equipment_item_id` in event_equipment queries

  2. Notes
    - event_equipment table has `equipment_id` not `equipment_item_id`
*/
      ...
      IF v_warehouse_category_id IS NOT NULL THEN
        FOR v_alt IN
          SELECT
            ...
            COALESCE((SELECT COUNT(*) FROM equipment_units
                      WHERE equipment_id = ei2.id
                      AND status IN ('available', 'in_use')), 0) as alt_total_qty,
            COALESCE((SELECT SUM(ee.quantity)::integer
                      FROM event_equipment ee
                      JOIN events e ON e.id = ee.event_id
                      WHERE ee.equipment_id = ei2.id
                      AND e.id != p_event_id
                      AND e.status != 'cancelled'
