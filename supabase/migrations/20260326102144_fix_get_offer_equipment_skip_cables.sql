/*
  # Fix get_offer_equipment_final - Skip Cables in Kit Expansion

  1. Problem
    - Function expanded kits including cables (where equipment_id IS NULL)
    - This caused NULL values in equipment_item_id which breaks logic

  2. Solution
    - Filter out cables when expanding kits
    - Only include kit items where equipment_id IS NOT NULL

  3. Notes
    - Cables from kits are managed separately in event_equipment
*/
...
COMMENT ON FUNCTION get_offer_equipment_final IS 'Zwraca finalny sprzęt z oferty: pojedyncze elementy + kity jako całości + rozwinięte elementy z kitów (pomijając przewody), z uwzględnieniem substytucji';
