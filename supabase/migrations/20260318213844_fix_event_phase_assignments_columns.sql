/*
  # Fix event_phase_assignments column names in timeline function

  1. Changes
    - Use invitation_status instead of status
    - Use assignment_start/assignment_end for date ranges
    - Properly handle null dates by falling back to phase times
*/
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(epa.invitation_status, 'confirmed')::text as item_status,
  ...
  WHERE epa.employee_id = ANY(p_employee_ids)
    AND COALESCE(epa.assignment_start, ep.start_time) <= p_end_date
    AND COALESCE(epa.assignment_end, ep.end_time) >= p_start_date
