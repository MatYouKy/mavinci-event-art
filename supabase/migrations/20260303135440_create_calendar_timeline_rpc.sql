/*
  # Create Calendar Timeline RPC Function

  1. Purpose
    - Single RPC function to fetch all timeline resources for calendar
    - Returns vehicles, employees, equipment and their assignments
    - Avoids complex nested REST queries with ambiguous foreign keys

  2. Returns
    - resource_type: 'vehicle' | 'employee' | 'equipment'
    - resource_id: UUID of the resource
    - resource_name: Display name
    - resource_metadata: JSONB with additional info (registration_number, category, etc)
    - event_id: UUID of assigned event
    - event_name: Name of event
    - event_start: Start datetime
    - event_end: End datetime
    - event_status: Status of event
    - assignment_status: Status of assignment (from event_vehicles.status, employee_assignments.invitation_status, etc)
*/
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(ep.work_start_datetime, e.event_start_datetime) as event_start,
    COALESCE(ep.work_end_datetime, e.event_end_datetime) as event_end,
    e.status::text as event_status,
    ev.status::text as assignment_status
  FROM vehicles v
  LEFT JOIN event_vehicles ev ON ev.vehicle_id = v.id
  LEFT JOIN events e ON e.id = ev.event_id AND e.deleted_at IS NULL AND e.status != 'cancelled'
  ...
  WHERE v.deleted_at IS NULL
    AND v.status IN ('available', 'in_use', 'maintenance')
    AND (
      ev.id IS NULL
      OR (
        COALESCE(ep.work_end_datetime, e.event_end_datetime) >= p_start_date
  ...
  SELECT
    ...
    e.id as event_id,
    e.name as event_name,
    e.event_start_datetime as event_start,
    e.event_end_datetime as event_end,
    e.status::text as event_status,
    'assigned'::text as assignment_status
  FROM employees emp
  LEFT JOIN event_employees ee ON ee.employee_id = emp.id
  LEFT JOIN events e ON e.id = ee.event_id AND e.deleted_at IS NULL AND e.status != 'cancelled'
  WHERE emp.deleted_at IS NULL
    AND emp.is_active = true
    AND (
      ee.id IS NULL
      OR (
        e.event_end_datetime >= p_start_date
        AND e.event_start_datetime <= p_end_date
      )
    )
  ...
  LEFT JOIN events e ON e.id = ea.event_id AND e.deleted_at IS NULL AND e.status != 'cancelled'
  LEFT JOIN event_phases ep ON ep.id = (
    SELECT id FROM event_phases
    WHERE event_id = e.id
    ORDER BY work_start_datetime ASC
  ...
  LEFT JOIN events e ON e.id = eeq.event_id AND e.deleted_at IS NULL AND e.status != 'cancelled'
  LEFT JOIN event_phases ep ON ep.id = (
    SELECT id FROM event_phases
    WHERE event_id = e.id
    ORDER BY work_start_datetime ASC
  ...
  WHERE ei.deleted_at IS NULL
    AND ei.status IN ('available', 'in_use')
    AND (
      eeq.id IS NULL
      OR (
        COALESCE(ep.work_end_datetime, e.event_end_datetime) >= p_start_date
