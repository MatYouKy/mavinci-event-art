/*
  # Napraw błąd "column reference event_date is ambiguous"
  
  Zmiana:
  - Użyj aliasów w SELECT aby uniknąć konfliktu nazw zmiennych z nazwami kolumn
*/
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND NEW.invitation_status IS DISTINCT FROM OLD.invitation_status THEN
    ...
    IF v_manager_ids IS NOT NULL THEN
      ...
      -- Wstaw odbiorców
      INSERT INTO notification_recipients (notification_id, employee_id)
      SELECT v_notification_id, unnest(v_manager_ids);
    END IF;
  END IF;
