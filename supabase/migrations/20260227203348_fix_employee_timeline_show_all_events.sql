/*
  # Fix Employee Timeline - Show All Events

  1. Changes
    - Fix event query to show events regardless of invitation status
    - Show only accepted invitations or events without invitation requirement
    - Fix phase handling to show event even without phases
    - Add better event grouping
*/
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    jsonb_build_object(
      ...
      'category', ec.name,
      'location', l.name,
      'invitation_status', ee.invitation_status
    ) as item_metadata
  FROM employees e
  ...
  WHERE e.id = ANY(p_employee_ids)
    AND (ee.invitation_status IS NULL OR ee.invitation_status IN ('pending', 'accepted'))
    AND (
  ...
  INNER JOIN event_employees ee ON ee.employee_id = e.id
  INNER JOIN events ev ON ev.id = ee.event_id
  INNER JOIN event_phases ep ON ep.event_id = ev.id
  LEFT JOIN event_categories ec ON ec.id = ev.category_id
  LEFT JOIN locations l ON l.id = ev.location_id
  WHERE e.id = ANY(p_employee_ids)
    ...
    AND (ee.invitation_status IS NULL OR ee.invitation_status IN ('pending', 'accepted'))
    AND (
      (ep.start_time >= p_start_date AND ep.start_time <= p_end_date)
      OR (ep.end_time >= p_start_date AND ep.end_time <= p_end_date)
      OR (ep.start_time <= p_start_date AND ep.end_time >= p_end_date)
    )
