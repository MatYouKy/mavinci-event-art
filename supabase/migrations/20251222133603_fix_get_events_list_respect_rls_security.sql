/*
  # Fix get_events_list to respect RLS
  
  1. Problem
    - Function uses SECURITY DEFINER which bypasses RLS
    - All users see all events regardless of their permissions
  
  2. Solution
    - Change to SECURITY INVOKER so function runs with caller's permissions
    - RLS policies will be respected
    - Users will only see events they have access to
  
  3. Security
    - Admins and calendar_manage: see all events
    - Other users: only events they created or are assigned to
*/
...
BEGIN
  result := (
    ...
    FROM (
      -- Events from events table (RLS will filter based on user permissions)
      SELECT
        jsonb_build_object(
          'id', e.id,
          'name', e.name,
      ...
      -- Meetings from meetings table (RLS will filter based on user permissions)
      SELECT
        jsonb_build_object(
          'id', m.id,
          'name', m.title,
...
COMMENT ON FUNCTION get_events_list IS 'Returns combined list of events and meetings for calendar display - respects RLS policies';
