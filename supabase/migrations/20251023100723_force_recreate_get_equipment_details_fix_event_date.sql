/*
  # Force recreate get_equipment_details with correct column names
  
  Changes:
  - DROP and recreate function to ensure old version is replaced
  - Use `created_at` instead of `event_date` in equipment_unit_events
*/

DROP FUNCTION IF EXISTS get_equipment_details(uuid);

CREATE FUNCTION get_equipment_details(equipment_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
