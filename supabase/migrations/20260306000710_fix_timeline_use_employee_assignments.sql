/*
  # Fix Timeline to Use employee_assignments

  Aktualizuje funkcję get_employee_timeline_data aby używała tabeli
  employee_assignments zamiast przestarzałej event_employees.

  Dodaje również invitation_status do metadanych i koloru.
*/
...
BEGIN
  RETURN QUERY
  -- Wydarzenia z employee_assignments i fazami
  SELECT
    e.id as employee_id,
    ...
    COALESCE(ep.start_time, ev.event_date) as item_start,
    COALESCE(ep.end_time, ev.event_end_date) as item_end,
    ev.status as item_status,
    -- Kolor zależy od invitation_status
    CASE
      WHEN ea.invitation_status = 'pending' THEN '#fbbf24'
      WHEN ea.invitation_status = 'accepted' THEN COALESCE(ep.color, ec.color, '#10b981')
